...
public class GreetingIntegrationTests {
	...
	public void corsWithAnnotation() {
		...
		ResponseEntity<Greeting> entity = this.restTemplate.exchange("/greeting", HttpMethod.GET, requestEntity,
				Greeting.class);
	...
	public void corsWithJavaconfig() {
		...
		headers.add(HttpHeaders.ORIGIN, "http://localhost:9000");
		HttpEntity<?> requestEntity = new HttpEntity(headers);
		ResponseEntity<Greeting> entity = this.restTemplate.exchange("/greeting-javaconfig", HttpMethod.GET,
				requestEntity, Greeting.class);
		assertEquals(HttpStatus.OK, entity.getStatusCode());
		assertEquals("http://localhost:9000", entity.getHeaders().getAccessControlAllowOrigin());
		Greeting greeting = entity.getBody();
		assertEquals("Hello, World!", greeting.getContent());
